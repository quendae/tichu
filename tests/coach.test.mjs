...
test('Coach explains how to answer a pair and validates a selected higher pair',()=>{
  ...
  const model=buildCoachModel(s,{...baseUi});
  assert.match(model.body,/wyższą parę|bomb/i);assert.equal(model.selectedValid,true);assert.match(model.selectedLabel,/para/i);
});
...
test('Coach explains that an active wish must be fulfilled when possible',()=>{
  ...
  const model=buildCoachModel(s,{...baseUi});
  assert.match(model.body,/8/);assert.match(model.body,/musisz|życzenie/i);assert.deepEqual([...model.legalCardIds].sort(),['jade-8','sword-8']);
});
...
test('Coach explains the current exchange recipient',()=>{
  ...
  assert.equal(model.title,'Przekaż kartę do Mei');assert.match(model.body,/rywala po lewej/i);
});
